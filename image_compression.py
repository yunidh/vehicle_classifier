...
for source_folder in source_folders:
    ...
    dest_folder = os.path.join(
        "data/vehicle_classifier_3Output/compressed", os.path.basename(source_folder)
    )
    ...
    image_files = [
        ...
        if any(f.endswith(ext) for ext in [".jpg", ".jpeg", ".JPG", ".JPEG"])
    ]
    ...
    for filename in image_files:
        ...
        # Save the compressed image with or without exif data
        if exif_data:
            ...
            compressed_image.save(
                os.path.join(dest_folder, filename), optimize=True, quality=85
            )
